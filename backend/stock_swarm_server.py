...
# Universe + relationship metadata is generated by the JS build pipeline
# into frontend/public/stock-swarm/stub.json. Vite copies public/ into
# dist/ on build, so in production the stub is reachable from the static
# dist directory; in dev we fall back to the public source. Override
# explicitly with STOCK_SWARM_STUB if you ship the file elsewhere.
def _resolve_stub_path():
    explicit = os.environ.get('STOCK_SWARM_STUB')
    if explicit:
        return Path(explicit)
    candidates = [
        STATIC_DIR / 'stock-swarm' / 'stub.json',
        Path(__file__).resolve().parent.parent / 'frontend' / 'public' / 'stock-swarm' / 'stub.json',
        Path(__file__).resolve().parent.parent / 'frontend' / 'dist' / 'stock-swarm' / 'stub.json',
    ]
    for c in candidates:
        if c.exists():
            return c
    return candidates[0]  # report the expected location even if missing
STUB_PATH = _resolve_stub_path()

_UNIVERSE_CACHE = None


def _load_universe():
    """Read the bundled stub.json for ticker metadata + fallback prices."""
    ...
    try:
        ...
        print(f'[stock-swarm] stub load failed: {exc}', flush=True)
        # Minimal degenerate universe — keeps the legacy 50 tickers alive
        # if the new bundle is missing for some reason.
        _UNIVERSE_CACHE = {
            'tickers': [
                {'symbol': sym, 'name': name, 'tier': tier, 'group': None,
                 'parents': [{'symbol': p, 'relation': 'peer', 'weight': w}
                              for p, w in PARENTS.get(sym, [])],
                 'children': []}
...
def _meta():
    """Return the canonical ticker metadata array (symbol/name/tier/group/parents/children)."""
    return _load_universe().get('tickers', [])


def _stub_payload():
    """Return the full bundled stub payload (universe metadata + synthetic prices)."""
...
def _fetch_yf(period):
    try:
        ...
        return None
    try:
        # Pull every ticker from the canonical universe (~438 symbols).
        meta = _meta()
        symbols = [t['symbol'] for t in meta]
        # yfinance uses '-' for class shares (BRK-B), but its query API wants
        # the dash form for some tickers. Pass through as-is; failures are
        # tolerated per-symbol below.
        df = yf.download(
        ...
        return {
            ...
            'stats': _load_universe().get('stats'),
            'tickers': meta,
            'prices': prices,
        }
    except Exception as exc:
...
def create_app():
    ...
    def prices():
        ...
        # yfinance unavailable: serve the bundled stub directly so the
        # frontend gets the new-schema universe + prices in one shot.
        stub = _stub_payload()
        out = dict(stub)
        out['source'] = 'stub'
        out['fallback_reason'] = 'yfinance_unavailable_or_failed'
        out['stub_url'] = '/stock-swarm/stub.json'
        return jsonify(out)
